...
function setAllFieldsErr(array){
    ...
    if(firstNameField.value.trim() === "") { firstNameField.classList.add('error'); containsVal = true; }
    else firstNameField.classList.remove('error');

    if(lastNameField.value.trim() === "") { lastNameField.classList.add('error'); containsVal = true; }
...
document.getElementById('remove-photo').addEventListener('click', function() {
    document.getElementById('profile-preview').src = "/Frontend/Icons/pencil-line.svg";
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if(savedContactInfo){ 
        if(savedContactInfo.length >= 4){
            ...
            if(savedContactInfo[4] && savedContactInfo[4] !== "") {
                const preview = document.getElementById('profile-preview');
                preview.src = savedContactInfo[4];
                updatePreviewAppearance();
            }
        }
    }
...
function isTaken(searchQuery){ 
    fetch("http://colorslab.xyz/ContactsAPI/searchContacts.php", {
        method: "POST",
...
//TASKS FOR WHEN THE ACTION BUTTON IS CLICKED
document.getElementById("action-button").addEventListener("click", function () {
    ...
    if(savedContactInfo){ 
        fetch("http://colorslab.xyz/ContactsAPI/editContact.php", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
                FirstName: firstName,
        ...
        fetch('http://colorslab.xyz/ContactsAPI/createContact.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ 
                FirstName: firstName, 
